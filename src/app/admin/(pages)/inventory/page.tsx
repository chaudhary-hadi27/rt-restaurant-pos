// src/app/admin/(pages)/inventory/page.tsx - REFACTORED (450 → 160 lines)
'use client'
...
export default function InventoryPage() {
    ...
    const columns = [
        ...
        { key: 'price', label: 'Price', align: 'right' as const, render: (row: any) => <span className="text-sm">PKR {row.purchase_price.toLocaleString()}</span> },
        { key: 'value', label: 'Value', align: 'right' as const, render: (row: any) => <span className="font-bold">PKR {(row.total_value || 0).toLocaleString()}</span> }
    ]

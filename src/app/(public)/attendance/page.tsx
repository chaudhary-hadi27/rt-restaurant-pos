    ...
    return (
        <div className="min-h-screen bg-[var(--bg)]">
            ...
            <div className="max-w-4xl mx-auto px-3 sm:px-4 lg:px-8 py-6 space-y-4">
                {waiters.map(waiter => (
                    <div key={waiter.id} className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 sm:p-6 flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 mt-1 rounded-full text-xs font-medium ${waiter.is_on_duty ? 'bg-green-500/20 text-green-600' : 'bg-gray-500/20 text-gray-600'}`}>
                        ...
                        <div className="flex-shrink-0">
                            {waiter.is_on_duty ? (
                                <button
                                    ...
                                    disabled={loading}
                                    className="px-4 sm:px-6 py-2 sm:py-3 bg-red-600 text-white rounded-lg font-medium flex items-center gap-2 hover:bg-red-700 active:scale-95 disabled:opacity-50 text-sm sm:text-base"
                                >
                                ...
                                <button
                                    ...
                                    disabled={loading}
                                    className="px-4 sm:px-6 py-2 sm:py-3 bg-green-600 text-white rounded-lg font-medium flex items-center gap-2 hover:bg-green-700 active:scale-95 disabled:opacity-50 text-sm sm:text-base"
                                >
